...
from camel.models.github_model_backend import GitHubModelBackend, GitHubModelError, GitHubAPIError
from camel.models.gemini_model_backend import GeminiModelBackend, GeminiModelError, GeminiAPIError
...
async def test_text_generation(github_model, gemini_model):
    ...
    github_response = await github_model.generate(prompt, max_tokens=100, temperature=0.7)
    github_time = time.time() - start_time
    assert isinstance(github_response, str)
    assert len(github_response) > 0
    
    start_time = time.time()
    gemini_response = await gemini_model.generate(prompt, max_tokens=100, temperature=0.7)
...
async def test_long_input_handling(github_model, gemini_model):
    ...
    start_time = time.time()
    github_response = await github_model.generate(long_prompt, max_tokens=50, temperature=0.7)
    github_time = time.time() - start_time
    assert isinstance(github_response, str)
    assert len(github_response) > 0

    start_time = time.time()
    gemini_response = await gemini_model.generate(long_prompt, max_tokens=50, temperature=0.7)
    gemini_time = time.time() - start_time
    assert isinstance(gemini_response, str)
    assert len(gemini_response) > 0
